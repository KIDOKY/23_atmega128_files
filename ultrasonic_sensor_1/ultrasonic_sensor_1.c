...
{   
    ...
    DDRA |= 0x01;        // 초음파센서 Trigger 신호( PA0 : 출력포트 설정  )
    PORTA &= ~0x01;      // PA0  : Low  ( Trigger 신호 OFF )  
    DDRE &= ~0x10;       // Echo 신호( 외부인터럽트4, INT4(PE4) : 입력포트 설정 )
   ...
   //////////////////////////////////////////////////////////////////////////////////////////
   // 외부인터럽트 4( pin: INT4/PE4 ) :  초음파센서모듈 Echo 신호 입력 
   // 교재 pp108-109 (레지스터 그림, 표 7.4-표7.5 참조 )

    EICRB &= ~0x02;        // INT4 : 하강에지(falling edge) 상승에지(rising edge) 모두에서 인터럽트 요구
                           // EICRB = 0b******01 
                           // EICRB = EICRB & ~0b00000010(~0x02 ) 
    EICRB |=   0x01;       // EICRB = EICRB | 0b00000001( 0x01 ) 
