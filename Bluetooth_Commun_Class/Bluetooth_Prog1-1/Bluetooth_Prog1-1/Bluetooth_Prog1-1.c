...
/////  < Bluetooth_Prog1=1.c >  : 블루투스모듈(HC-06)을 이용하여 스마트폰과 통신하는 ATmega128 코드  ///// 
...
/////////  USART1 통신 포트 설정 (344쪽 - 351쪽 관련 레지스터와 표 참조 )  //////////////////   
    ...
    { 
	   ...
	   { 
	       ...
	       if( rdata == 'a' )          // 수신된 문자 데이터가 ‘a’ 이면 
	       {
               PORTA |= 0x01;          // LED1 OFF 
	       }

	       else if( rdata == 'b' )     // 수신된 문자 데이터가 'b' 이면
	       {
               PORTA &= ~0x01;         // LED1 ON
	       }

	       else if( rdata == 'c' )     // 수신된 문자 데이터가 'c' 이면
	       {
                    PORTA ^= 0x01;         // LED1 Toggle
	       }

	       else if( rdata == 'd')      // 수신된 문자데이터가 ‘d’ 이면
	       {
